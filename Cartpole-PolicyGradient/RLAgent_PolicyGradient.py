...
class PolicyGradient:
    ...
    def _BuildNet(self):
        ...
        DeltaValue = Input([], dtype=tf.float32, name="ActionValue")
        net = Dense(hiddenUnits,
        ...
        self.model = Model(inputs=[state, DeltaValue], outputs=ActionProb)
    ...
    def chooseAction(self, state):
        # InputData = {self.state: [observation]}
        # actionProb = self.session.run(self.ActionProb, feed_dict= InputData)
        # print(state)
        # print(state.shape)
        # assert False

        actionProb = self.model.predict([
            state.reshape([-1, self.numFeatures]),
            np.zeros([1])
        ])
        ...
        actionValue = np.random.choice(self.numActions, p=actionProb[0])
        return actionValue
    ...
    def learn(self):
        ...
        #     })
        values = np.array(self.MemoryAction)
        n_values = np.max(values) + 1
        oneHotAction =np.eye(n_values)[values].astype(np.float32)
        self.model.fit([np.array(self.MemoryObservation), np.array(normalizedReward)],
                       oneHotAction,
                       epochs=1,
                       verbose=0
                       )
